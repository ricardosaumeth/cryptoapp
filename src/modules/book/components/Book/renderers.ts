...
export const bidAmountRenderer = (params: ICellRendererParams) => {
    ...
    return amountRenderer(valueFormatted as string, { 'background-color': Palette.BidTransparent, width: `${width}%`, left: '-12px' });
};
...
export const askAmountRenderer = (params: ICellRendererParams) => {
  const {valueFormatted} = params
    ...
    const width = ((depth || 0) / maxDepth) * 100;
    return amountRenderer(valueFormatted as string, { 'background-color': Palette.AskTransparent, width: `${width}%`, right: '-12px' });
};